...
**Context**: Ran a security-focused review of the app. Code-side findings were fixed on branch `bugfix/security-hardening` (3 commits): #1 profiles no longer store the raw email in `display_name` (migration + backfill), #5 client-side minimum password length (8) at sign-up, #2 `ai-chat` prompt-length cap (4000 chars), #4 `ai-chat` treats the user-supplied recipe catalog as untrusted data against prompt injection. Swift suite verified (78 tests). The SQL migration and Edge Function changes were implemented but not executed in that environment (no Docker/Deno) — apply/redeploy to confirm live.

**Outstanding — these are hosted-project / Dashboard actions that CANNOT be done in code, and MUST be completed before inviting a second real user or shipping** (they compound with the shared-recipe-box model, where any authenticated user can read all recipes and every profile):

- [ ] **#3 — Close open self-registration.** Hosted Auth currently allows anyone to sign up and (with `mailer_autoconfirm = true`) get an active session immediately, granting full access to the shared box. Make it invite-only / disable open signup, re-enable email confirmation (`mailer_autoconfirm = false`), and add a captcha (hCaptcha/Turnstile). Ties into the existing `mailer_autoconfirm` flip-back note (2026-07-06).
- [ ] **#6 — Rotate the `service_role` key.** It was printed into a chat transcript (see 2026-07-05 note) and bypasses RLS entirely. The app never uses it, so rotation (Settings → API) is zero-risk to the client. Do this before any real data lands or the repo/transcript is shared.
- [ ] **#5 (hosted half) — Raise password policy on the hosted project.** Set `minimum_password_length` ≥ 8 and a `password_requirements` tier (and enable leaked-password protection) in the hosted Auth settings. The client + local `config.toml` were already raised to 8; the hosted project is configured separately and must match.
